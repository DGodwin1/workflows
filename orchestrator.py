...
class Orchestrator:
    ...
    async def do_workflow(self):
        ...
        match self.pattern:
            case "concurrent":
                return await run_prompt_and_print_output(agent_framework.ConcurrentBuilder().participants(self.participants).build(), prompt)
            case "sequential":
                return await run_prompt_and_print_output(agent_framework.SequentialBuilder().participants(self.participants).build(), prompt)
